...
**StockLine** is a tiny inventory app that predicts the **death line** of every item: the last day you can place an order and still receive it before you run out. On top of that forecast, **Claude** prioritises what to buy, logs stock movements from plain text, reads delivery notes from a photo and answers questions about your stock.
...
| 🧠 | **AI analysis**: what to order today, how much, and what looks odd (dead stock, usage spikes, rising trends) | Claude + structured outputs (JSON schema) |
| 💬 | **Plain-text logging**: *"took 3 headphones and 2 USB-C cables for the Girona office"* → movements | Claude + structured outputs; you confirm before anything changes |
| 📸 | **Delivery note → stock**: take a photo of a delivery note or invoice and the lines become stock entries | Claude vision; new products are detected and created |
| 🗨️ | **Chat with your inventory**, including ready-to-send purchase order drafts | Claude, streamed token by token |
...
- Claude receives this already-computed snapshot, so it reasons about decisions instead of doing arithmetic.
...
- **Live mode**: paste your own [Anthropic API key](https://console.anthropic.com/settings/keys) in ⚙️ Settings. It's stored **only in your browser** and sent straight to `api.anthropic.com` via the official SDK (`dangerouslyAllowBrowser`). Set a spend limit on the key, and don't use it on shared computers.

The default model is **Claude Opus 5**. You can switch the model in Settings, and set the reasoning effort per feature in `config.js`. If the model declines a request, the API automatically retries it on Anthropic's recommended fallback model (`fallbacks: "default"`).
...
| `ai.model`, `ai.models`, `ai.effort` | Default model, models offered in Settings, effort per feature |
...
js/
  ...
  ai.js                 Claude API: prompts, schemas, streaming, errors
